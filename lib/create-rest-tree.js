...
*/
export function calculateBranchNodes (records, indexPath, propNamesPath, acc) {
...
export function normalizeRecord (record, props) {
  ...
  const mergeUrls = (urls) => {
    return urls[urls.length - 1] = urls
      .filter(url => (url !== ''))
      .reduce((acc, url) => {
        const last = acc[acc.length - 1] // string contain url
        if (!last.endsWith('/') && !url.startsWith('/')) {
          acc.push('/' + url)
        }
        return acc
      }, [''])
  ...
  const urls = normalizedRecord.options.map(({ url = '' }) => url)
  const lastIndex = normalizedRecord.options.length - 1
  if (!urls[lastIndex].startsWith('/')) {
    normalizedRecord.options[lastIndex].url = mergeUrls(urls)
  }
